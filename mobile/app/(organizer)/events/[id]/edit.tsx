  ...
  useEffect(() => {
    ...
    getOrganizerEvent(id)
      .then(({ event }) => {
        ...
        setDateStart(formatDateForInput((event as any).date_start))
        setDateEnd(formatDateForInput((event as any).date_end))
        setVenueName((event as any).venue_name ?? '')
        setVenueAddress((event as any).venue_address ?? '')
        setVenueCity((event as any).venue_city ?? '')
        setCapacity((event as any).capacity ? String((event as any).capacity) : '')
        setTicketPrice((event as any).ticket_price ? String((event as any).ticket_price) : '')
        setRegMode(((event as any).registration_mode ?? 'open') as RegMode)
        setPublishNow((event as any).status === 'published')
      })
        ...
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={s.publishRow}>
            ...
            <Switch
              ...
              accessibilityLabel="Published status"
            />
          </View>

          <TouchableOpacity
            style={[s.createBtn, { backgroundColor: colors.blue }, saving && { opacity: 0.6 }]}
            onPress={handleSave}
            disabled={saving}
            activeOpacity={0.85}
            accessibilityLabel="Save changes"
