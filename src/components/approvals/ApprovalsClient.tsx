...
export default function ApprovalsClient({
 ...
 if (!res.ok) {
  ...
  const reject = async (reg: Registration) => {
    ...
    await supabase.from('public_registrations')
    ...
    <div className="space-y-6 max-w-4xl">
          ...
          {publicEvents.map(event => (
            <div key={event.id} className="flex items-center justify-between p-3 rounded-lg bg-brand-charcoal-light border border-white/5">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => copyLink(event.id)}
                ...
                <a href={`/register/${event.id}`} target="_blank" rel="noopener noreferrer" className="btn-secondary text-xs px-3 py-1.5 flex items-center gap-1.5">
      ...
      <div className="flex gap-3">
        <div className="relative">
          <select className="input appearance-none pr-10" value={selectedEventId} onChange={e => setSelectedEventId(e.target.value)}>
              ...
              <div key={reg.id} className="card flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-500">{reg.email}{reg.phone ? ` · ${reg.phone}` : ''}{reg.gender ? ` · ${reg.gender}` : ''}</p>
                    <p className="text-xs text-gray-600 mt-0.5">{event?.title} · {new Date(reg.created_at).toLocaleDateString('en-PK', { dateStyle: 'medium' })}</p>
                  </div>
                </div>
