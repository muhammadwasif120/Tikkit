...
function StatusBadge({ status, paymentStatus }: { status: string; paymentStatus: string }) {
  ...
  else if (status === 'pending')                                      { label = 'Pending Review';     color = 'var(--text-muted)'; bg = 'rgba(107,114,128,0.1)'; border = 'rgba(107,114,128,0.15)'; Icon = Clock         }
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ background: 'var(--surface-card)', border: '1px solid var(--guest-border)' }}
      >
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            <h2 style={{ color: 'var(--text-primary)', fontSize: 'var(--fs-xl)', fontWeight: 800, margin: '0 0 6px', fontFamily: 'var(--font-display)' }}>{reg.full_name}</h2>
            <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-base)', margin: 0 }}>{event?.title ?? 'Unknown event'}</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <StatusBadge status={reg.status} paymentStatus={reg.payment_status} />
            <button onClick={onClose} style={{ background: 'var(--guest-surface-2)', border: '1px solid var(--guest-border)', color: 'var(--text-secondary)', cursor: 'pointer', width: 32, height: 32, borderRadius: 8, fontSize: 'var(--fs-xl)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
          </div>
        </div>

        {/* Applicant info */}
        <div style={{ background: 'var(--guest-surface)', border: '1px solid var(--guest-border)', borderRadius: 14, padding: '16px', marginBottom: 12 }}>
          <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', margin: '0 0 12px' }}>Applicant Details</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              ...
              <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-base)', width: 60, flexShrink: 0 }}>{label}</span>
                <span style={{ color: 'var(--text-primary)', fontSize: 'var(--fs-base)' }}>{value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {event?.require_reference_code && (
          <div style={{ background: 'var(--guest-surface)', border: '1px solid var(--guest-border)', borderRadius: 14, padding: '14px 16px', marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <Tag size={13} color="var(--text-muted)" />
              <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', margin: 0 }}>Reference Code</p>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <code style={{ color: 'var(--text-primary)', fontSize: 'var(--fs-md)', fontFamily: 'monospace', background: 'var(--guest-surface-2)', padding: '4px 10px', borderRadius: 6 }}>
        ...
        {reg.id_document_url && (
          <div style={{ background: 'var(--guest-surface)', border: '1px solid var(--guest-border)', borderRadius: 14, padding: '14px 16px', marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', margin: 0 }}>ID Document</p>
            </div>
            ...
            <img
              ...
              style={{ width: '100%', maxHeight: 200, objectFit: 'cover', borderRadius: 10, cursor: 'zoom-in', border: '1px solid var(--guest-border)' }}
            />
          </div>
        )}
        ...
        {reg.payment_screenshot_url && (
          <div style={{ background: 'var(--guest-surface)', border: '1px solid rgba(255,199,69,0.2)', borderRadius: 14, padding: '14px 16px', marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <img
              ...
              style={{ width: '100%', maxHeight: 240, objectFit: 'cover', borderRadius: 10, cursor: 'zoom-in', border: '1px solid var(--guest-border)', marginBottom: 8 }}
            />
            <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-sm)', margin: 0 }}>Tap to zoom</p>
          </div>
        )}
        ...
        {reg.notes && (
          <div style={{ background: 'var(--guest-surface)', border: '1px solid var(--guest-border)', borderRadius: 14, padding: '14px 16px', marginBottom: 12 }}>
            <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', margin: '0 0 6px' }}>Applicant Note</p>
            <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--fs-base)', margin: 0, lineHeight: 1.6 }}>{reg.notes}</p>
          </div>
        )}
        ...
        {showReject && (
          <div style={{ marginBottom: 16 }}>
            <label style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-sm)', fontWeight: 600, display: 'block', marginBottom: 6 }}>Reason for rejection (optional)</label>
            <textarea
              value={rejectNote} onChange={e => setNote(e.target.value)}
              placeholder="Let the applicant know why they were declined..."
              rows={3}
              style={{ width: '100%', padding: '10px 12px', background: 'var(--guest-surface)', border: '1px solid var(--guest-border)', borderRadius: 10, color: 'var(--text-primary)', fontSize: 'var(--fs-base)', resize: 'none', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
            />
          </div>
        )}
        ...
        {canApprove && (
          <div style={{ display: 'flex', gap: 10 }}>
            {!showReject ? (
              ...
              <>
                <button onClick={() => setShowRej(false)} style={{ flex: 1, padding: '12px', background: 'var(--guest-surface-2)', border: '1px solid var(--guest-border)', borderRadius: 12, color: 'var(--text-muted)', fontSize: 'var(--fs-md)', fontWeight: 600, cursor: 'pointer' }}>
                  Cancel
                </button>
                <button
...
function StatusIcon({ status, paymentStatus }: { status: string; paymentStatus: string }) {
  let Icon = Clock, color = 'var(--text-muted)'
    ...
    <div className="max-w-5xl px-0 pt-2 pb-6 sm:px-6 sm:pt-7">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 20 }}>
        <div style={{
          ...
          background: 'linear-gradient(135deg, rgba(var(--brand-blue-rgb),0.2), rgba(168,85,247,0.12))',
          border: '1px solid rgba(var(--brand-blue-rgb),0.25)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: '0 8px 24px rgba(var(--brand-blue-rgb),0.15)',
        }}>
          <ClipboardCheck size={22} color="var(--brand-blue)" />
        </div>
        <div>
      ...
      {/* ── Search + event filter ────────────────────────────── */}
      <div style={{ background: 'var(--guest-surface)', border: '1px solid var(--guest-border)', borderRadius: 16, padding: '16px', marginBottom: 10 }}>
          ...
          <div style={{ position: 'relative' }}>
            <select
              ...
              style={{
                ...
                background: 'var(--guest-surface-2)', border: '1px solid var(--guest-border)',
                borderRadius: 12, color: 'var(--text-primary)', fontSize: 'var(--fs-base)', cursor: 'pointer',
          ...
          return (
            <button
              ...
              style={{
                ...
                background: isActive
                  ? isUrgent ? 'rgba(255,199,69,0.1)' : 'rgba(var(--brand-blue-rgb),0.12)'
                  : 'var(--guest-surface)',
                border: isActive
                  ? isUrgent ? '1px solid rgba(255,199,69,0.3)' : '1px solid rgba(var(--brand-blue-rgb),0.3)'
                  : '1px solid var(--guest-border)',
              }}
            >
              <span style={{
                fontSize: 'var(--fs-sm)', fontWeight: 700,
                color: isActive ? (isUrgent ? '#FFC745' : 'var(--text-primary)') : 'var(--text-muted)',
              }}>
              ...
              <span style={{
                ...
                background: isActive
                  ? isUrgent ? 'rgba(255,199,69,0.2)' : 'rgba(var(--brand-blue-rgb),0.2)'
                  : 'var(--guest-surface-2)',
                color: isActive
      ...
      {/* ── Registration list ────────────────────────────────── */}
      <div style={{ background: 'var(--guest-surface)', border: '1px solid var(--guest-border)', borderRadius: 18, overflow: 'hidden' }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '72px 24px' }}>
            <div style={{
              width: 56, height: 56, borderRadius: 18, margin: '0 auto 16px',
              background: 'rgba(var(--brand-blue-rgb),0.1)', border: '1px solid rgba(var(--brand-blue-rgb),0.2)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
              <ClipboardCheck size={24} color="var(--brand-blue)" />
            </div>
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                        ...
                        <p style={{ color: 'var(--text-primary)', fontSize: 'var(--fs-base)', fontWeight: 700, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{reg.full_name}</p>
                      </div>
                      <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{eventMap[reg.event_id]?.title ?? '—'}</p>
                    </div>
                      ...
                      >
                        <td style={{ padding: '14px 20px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            {needsAttention && <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#FFC745', flexShrink: 0 }} />}
                            <span style={{ color: 'var(--text-primary)', fontSize: 'var(--fs-base)', fontWeight: 700 }}>{reg.full_name}</span>
                          </div>
                        </td>
                        <td style={{ padding: '14px 20px', color: 'var(--text-muted)', fontSize: 'var(--fs-base)' }}>{event?.title ?? '—'}</td>
                        <td style={{ padding: '14px 20px', color: 'var(--text-muted)', fontSize: 'var(--fs-base)' }}>{reg.email}</td>
                        <td style={{ padding: '14px 20px' }}>
                          <StatusBadge status={reg.status} paymentStatus={reg.payment_status} />
                        </td>
                      </tr>
      ...
      <style>{`
        .approval-row:hover { background: var(--guest-surface-2) !important; }
      `}</style>
    </div>
  )
}
