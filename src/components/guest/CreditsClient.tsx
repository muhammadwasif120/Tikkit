...
const TIERS: TierInfo[] = [
  { name: 'Newcomer', color: 'var(--text-muted)', minScore: 0,    maxScore: 49,   perks: ['Basic access to public events', 'Collect attendance passes'] },
  { name: 'Rising',   color: '#22C55E', minScore: 50,   maxScore: 199,  perks: ['Priority waitlist placement', 'Early event notifications'] },
  { name: 'Regular',  color: 'var(--brand-blue)', minScore: 200,  maxScore: 499,  perks: ['Early access to select events', 'Regular member badge'] },
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'right' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: 10, margin: 0 }}>{fmtTime(tx.created_at)}</p>
      </div>
    </div>
  )
}
...
export default function CreditsClient({
  ...
  return (
    <div style={{ padding: '16px' }}>
      {/* Score card */}
      <div style={{
        background: `linear-gradient(135deg, ${tier.color}20 0%, var(--surface-card) 100%)`,
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            <p style={{ color: 'var(--text-muted)', fontSize: 11, fontWeight: 700, letterSpacing: '0.8px', textTransform: 'uppercase', margin: '0 0 4px' }}>
              Credit Score
            </p>
            <p style={{
              color: 'var(--text-primary)', fontSize: 48, fontWeight: 900,
              fontFamily: 'var(--font-display)',
        ...
        {nextTier && (
          <div>
            ...
            <div style={{ height: 6, borderRadius: 3, background: 'var(--guest-border)', overflow: 'hidden' }}>
              <div style={{
      ...
      <div style={{
        background: 'var(--surface-card-2)', border: '1px solid var(--guest-border)',
        borderRadius: 16, padding: '14px', marginBottom: 12,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            <Shield size={14} color={tier.color} />
            <span style={{ color: 'var(--text-primary)', fontSize: 13, fontWeight: 700 }}>Your Perks</span>
          </div>
          <button
            ...
            style={{
              ...
              color: 'var(--brand-blue)', fontSize: 12, fontWeight: 600,
            }}
          >
        ...
        {tier.perks.map(perk => (
          <div key={perk} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0' }}>
            ...
            <span style={{ color: 'var(--text-muted)', fontSize: 13 }}>{perk}</span>
          </div>
        ))}
      </div>
      ...
      {showTiers && (
        <div style={{
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 11, fontWeight: 700, letterSpacing: '0.8px', textTransform: 'uppercase', margin: '0 0 12px' }}>
            All Tiers
          </p>
          {TIERS.map(t => {
            ...
            return (
              <div key={t.name} style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                    <span style={{ color: active ? t.color : 'var(--text-primary)', fontSize: 13, fontWeight: 700 }}>{t.name}</span>
                    {active && (
