...
async function verifyVendorOwnership(supabase: any, vendorId: string, userId: string) {
  const { data } = await (supabase as any)
    .from('vendors').select('id').eq('id', vendorId).eq('organizer_id', userId).single()
  return !!data
}
...
export async function GET(req: NextRequest) {
  ...
  const { data: vendors } = await (supabase as any)
    .from('vendors').select('id').eq('organizer_id', userId)
...
export async function PATCH(req: NextRequest) {
  ...
  const { data: inv } = await (supabase as any)
    .from('vendor_invoices').select('id, vendor_id').eq('id', id).single()
